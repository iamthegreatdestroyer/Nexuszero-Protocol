...
$lines = Get-Content -Raw -LiteralPath $File -Encoding UTF8 -ErrorAction Stop -ReadCount 0 | Out-String -Stream

$convertedLines = @()
foreach ($line in $lines) {
    $trim = $line.TrimEnd()`
    switch -Wildcard ($trim) {
        "`powershell" { $convertedLines += '```powershell'; continue }
        "`bash" { $convertedLines += '```bash'; continue }
        "`text" { $convertedLines += '```text'; continue }
        "`python" { $convertedLines += '```python'; continue }
        "`" { # solitary backtick: convert to closing fence
            $convertedLines += '```'; continue
        }
        default { $convertedLines += $line }
    }
}
